...
suite('keybindingIO', () => {
	...
	test('systemWide - write/export preserves the flag (roundtrip)', () => {
		...
		const item = new ResolvedKeybindingItem(resolvedKeybinding, 'workbench.action.openSettings', undefined, undefined, false, null, false, /* systemWide */ true);
		...
		assert.strictEqual(readBack.command, 'workbench.action.openSettings');
	});
});
