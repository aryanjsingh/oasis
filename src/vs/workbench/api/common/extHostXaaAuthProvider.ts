...
export function XaaifyAuthProvider<TBase extends Ctor<DynamicAuthProvider>>(Base: TBase): TBase {
	return class XaaAuthenticationProvider extends Base {
		...
		override async getSessions(scopes: readonly string[] | undefined, options: vscode.AuthenticationProviderSessionOptions): Promise<vscode.AuthenticationSession[]> {
			...
			try {
				const minted = await this._mintResourceToken(idpSession, [...scopes], audience, resource, options);
				if (!minted) {
					return [];
				}
		...
		override async createSession(scopes: string[], options: vscode.AuthenticationProviderSessionOptions): Promise<vscode.AuthenticationSession> {
			...
			if (!resource) {
				throw new Error('Enterprise-managed authentication requires `options.resource` (the resource indicator) but none was provided.');
			}
			...
			if (!minted) {
				...
				// Today the only such path is a missing resource client_secret.
				throw new Error('Failed to mint a resource access token for the enterprise-managed resource server.');
			}
			return toSession(minted.token, minted.scopes);
		}
		 ...
		 * and no client_secret can be resolved, this returns `undefined`.
		 *
		...
		private async _mintResourceToken(
			...
			// reusing `this._clientId`. Caller-supplied `options.clientId` takes precedence over the
			// JAG-extracted value.
			...
			// directly via `options.clientSecret`; otherwise we can only reuse a cached value.
			...
			if (configuredResourceClientSecret) {
				...
				if (this._resourceClientSecrets.has(secretCacheKey)) {
					...
					this._logger.info(`[XAA] Resource mint requires resource client_secret for '${resourceClientId}' but none is cached or configured.`);
					return undefined;
				}
			}
