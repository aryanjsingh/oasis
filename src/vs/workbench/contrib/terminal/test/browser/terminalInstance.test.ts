...
suite('Workbench - TerminalInstance', () => {
	...
	suite('TerminalLabelComputer', () => {
		...
		test('uses process name when no override is provided', () => {
			const terminalLabelComputer = createLabelComputer({ terminal: { integrated: { tabs: { separator: ' - ', title: '${process}', description: '${cwd}' } } } });
			terminalLabelComputer.refreshLabel(createInstance({ capabilities, processName: 'process' }));
			strictEqual(terminalLabelComputer.title, 'process');
		});
		test('uses shell type placeholder when configured', () => {
			const terminalLabelComputer = createLabelComputer({ terminal: { integrated: { tabs: { separator: ' - ', title: '${shellType}', description: '${cwd}' } } } });
			terminalLabelComputer.refreshLabel(createInstance({ capabilities, shellType: PosixShellType.Bash, processName: 'bash' }));
			strictEqual(terminalLabelComputer.title, 'bash');
		});
		test('uses title template from config when provided', () => {
			const terminalLabelComputer = createLabelComputer({ terminal: { integrated: { tabs: { separator: ' - ', title: '${process}', description: '${cwd}' } } } });
			terminalLabelComputer.refreshLabel(createInstance({ capabilities, processName: 'process', shellLaunchConfig: { titleTemplate: '${process}' }, shellType: PosixShellType.Bash }));
			strictEqual(terminalLabelComputer.title, 'process');
		});
		test('uses title template from config with fallback', () => {
			const terminalLabelComputer = createLabelComputer({ terminal: { integrated: { tabs: { separator: ' - ', title: '${process}', description: '${cwd}' } } } });
			terminalLabelComputer.refreshLabel(createInstance({ capabilities, processName: 'process', shellLaunchConfig: { titleTemplate: '${process}' } }));
			strictEqual(terminalLabelComputer.title, 'process');
		});
