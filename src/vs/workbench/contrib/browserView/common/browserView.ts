...
export interface IBrowserViewWorkbenchService {
	 ...
	 * group (or leave it unspecified) are redirected to the main editor area so
	 * the browser docks instead of opening as a modal overlay. Explicit placements
	 * (side group, auxiliary window, a specific group) are left untouched.
	 */
...
export class BrowserViewModel extends Disposable implements IBrowserViewModel {
	...
	constructor(
		...
		this._register(this.onDidChangeElementSelectionActive(active => this._isElementSelectionActive = active));
