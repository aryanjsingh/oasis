...
class TerminalInitialHintWidget extends Disposable {
	...
	constructor(
		...
		this._toDispose.add(_instance.onDidFocus(() => {
			if (this._instance.hasFocus && this._isVisible && this._ariaLabel) {
	...
	getDomNode(): HTMLElement | undefined {
		if (!this._domNode) {
			...
			this._ariaLabel = ariaLabel.concat(localize('disableHint', ' Toggle terminal.initialHint in settings to disable this hint.'));
