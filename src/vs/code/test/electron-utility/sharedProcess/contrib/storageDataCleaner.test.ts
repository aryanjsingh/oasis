...
suite('UnusedWorkspaceStorageDataCleaner', () => {
	...
	test('cleans unused workspace storage', async () => {
		const unusedWorkspace = URI.file(join(testDir, 'unused.code-workspace'));
		const unusedWorkspaceFolder = getWorkspaceIdentifier(unusedWorkspace).id;
		...
		for (const folder of [unusedWorkspaceFolder, md5LikeFolder, 'ext-dev', otherEmptyFolder]) {
