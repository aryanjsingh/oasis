...
suite('KeybindingResolver', () => {
	...
	suite('handle keybinding removals', () => {
		...
		test('issue #293802: removal still matches when default when clause becomes more specific', () => {
			const defaults = [
				kbItem(KeyCode.KeyA, 'command1', null, ContextKeyExpr.and(ContextKeyExpr.has('inSpecialInput'), ContextKeyExpr.not('withinEditSessionDiff')), true),
			];
			const overrides = [
				kbItem(KeyCode.KeyA, '-command1', null, ContextKeyExpr.has('inSpecialInput'), false),
			];
			const actual = KeybindingResolver.handleRemovals([...defaults, ...overrides]);
			assert.deepStrictEqual(actual, []);
		});
		...
		test('removal with more specific when clause does not match broader default', () => {
			const defaults = [
				kbItem(KeyCode.KeyA, 'command1', null, ContextKeyExpr.has('inSpecialInput'), true),
			];
			const overrides = [
				kbItem(KeyCode.KeyA, '-command1', null, ContextKeyExpr.and(ContextKeyExpr.has('inSpecialInput'), ContextKeyExpr.not('withinEditSessionDiff')), false),
			];
			const actual = KeybindingResolver.handleRemovals([...defaults, ...overrides]);
			assert.deepStrictEqual(actual, [
				kbItem(KeyCode.KeyA, 'command1', null, ContextKeyExpr.has('inSpecialInput'), true),
