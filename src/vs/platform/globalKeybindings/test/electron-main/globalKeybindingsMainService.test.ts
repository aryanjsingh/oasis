...
suite('GlobalKeybindingsMainService', () => {
	...
	test('trigger dispatches the run-action payload without force-focusing the routing window', () => {
		...
		service.updateKeybindings(1, [binding('Control+Cmd+A', 'workbench.action.openSettings', { foo: 'bar' })]);

		shortcut.trigger('Control+Cmd+A');

		// The command controls what is surfaced/focused, so the
		...
		assert.deepStrictEqual(window.sent, [{
			...
			args: [{ id: 'workbench.action.openSettings', from: 'systemWideKeybinding', args: [{ foo: 'bar' }] }]
		}]);
	});
