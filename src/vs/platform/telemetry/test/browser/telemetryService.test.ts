...
suite('TelemetryService', () => {
	...
	test('setCommonProperty adds property to all subsequent events', function () {
		...
		service.setCommonProperty('common.sampleTrackingId', 'test-tracking-id');
		service.publicLog('eventAfterSet');

		assert.strictEqual(testAppender.events[0].data['common.sampleTrackingId'], undefined);
		assert.strictEqual(testAppender.events[1].data['common.sampleTrackingId'], 'test-tracking-id');
