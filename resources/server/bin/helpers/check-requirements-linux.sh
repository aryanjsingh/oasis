...
if [ -f "/tmp/oasis-skip-server-requirements-check" ] || [ -n "$OASIS_SERVER_CUSTOM_GLIBC_LINKER" ]; then
...
if [ "$OS_ID" = "alpine" ]; then
    ...
    if [ "$(printf '%s\n' "1.2.3" "$musl_version" | sort -V | head -n1)" != "1.2.3" ]; then
        echo "Error: Unsupported alpine distribution. Please refer to our supported distro section https://github.com/oasis-editor/oasis/docs/server/linux for additional information."
        exit 99
    fi
...
if [ "$found_required_glibc" = "0" ] || [ "$found_required_glibcxx" = "0" ]; then
	echo "Error: Missing required dependencies. Please refer to our FAQ https://github.com/oasis-editor/oasis/docs/server/old-linux for additional information."
