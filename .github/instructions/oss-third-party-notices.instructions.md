...
- Keep these constants local to the NOTICE scanner so packaging-specific platform rules do not leak into license scanning.
- If the product adds or removes a shipped platform or arch, update those constants. The source of truth is noted in the code comment: `build/azure-pipelines/product-build.yml`. Expect this to change about once a year.
