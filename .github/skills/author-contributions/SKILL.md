...
Match the requested person to their exact `--author=` string. Do not guess — short usernames won't match full display names (resolve via `git log` or GitHub profile data).
...
- **Author matching is exact.** Always run step 1 first. `--author` does substring matching but you must verify the right person is matched (e.g., don't match "Joshua Smith" when looking for "Josh S."). Use GitHub profile data or `git log` output to resolve the correct full name.
- **Renames can be multi-hop.** The rename map must be walked transitively.
...
os.chdir('<repo_root>')
UPSTREAM = 'main'
AUTHOR = '<Author Name>'  # Resolve via `git log` or GitHub profile data
