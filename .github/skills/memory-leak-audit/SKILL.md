...
**Validated by**: Pattern used in `fileBasedRecommendations.ts` and `testingContentProvider.ts`.
...
4. Re-run the scenario with verbose heap/DOM sampling when the leak only appears under interaction.
