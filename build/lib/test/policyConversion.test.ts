...
suite('Policy E2E conversion', () => {
	...
	test('ObjectPolicy.from accepts a union type (e.g. array | null)', () => {
		...
		const policy: PolicyDto = {
			key: 'extensions.extraGalleryUrls',
			name: 'ExtensionExtraGalleryUrls',
			category: 'Extensions',
			minimumVersion: '1.0',
	...
	test('descriptions containing angle brackets are escaped in ADML output (#320551)', () => {
		const policyData: ExportedPolicyDataDto = {
			...
			policies: [
				{
					key: 'extensions.enabledSources',
					name: 'ExtensionEnabledSources',
					category: 'Extensions',
					minimumVersion: '1.122',
					localization: {
						description: {
							key: 'extensions.enabledSources.policy',
							value: 'Source enablement. Keys are source IDs in `<source>@<marketplace>` form; values enable or disable the source.'
						}
					},
					type: 'object',
					default: {},
				...
				{
					key: 'extensions.extraMarketplaces',
					name: 'ExtensionExtraMarketplaces',
					category: 'Extensions',
					minimumVersion: '1.122',
					localization: {
						description: {
							key: 'extensions.extraMarketplaces.policy',
		...
		assert.ok(enUs.contents.includes('&lt;source&gt;@&lt;marketplace&gt;'), 'angle brackets in descriptions must be XML-escaped');
		assert.ok(enUs.contents.includes('&lt;url&gt;'), 'angle brackets in descriptions must be XML-escaped');
		assert.ok(!enUs.contents.includes('<source>'), 'raw angle brackets must not appear in ADML output');
