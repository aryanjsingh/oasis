...
function buildWin32Setup(arch: string, target: string): task.CallbackTask {
	...
	return (cb) => {
		...
		if (quality === 'stable' || quality === 'insider') {
			definitions['AppxPackage'] = `${quality === 'stable' ? 'oasis' : 'oasis_insider'}_${arch}.appx`;
			definitions['AppxPackageDll'] = `${quality === 'stable' ? 'oasis' : 'oasis_insider'}_explorer_command_${arch}.dll`;
