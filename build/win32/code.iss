...
AppPublisher=Oasis
AppPublisherURL=https://github.com/oasis-editor/oasis/
AppSupportURL=https://github.com/oasis-editor/oasis/
AppUpdatesURL=https://github.com/oasis-editor/oasis/
...
// to shutdown Oasis. We rely on the fact that the update signals
// that Oasis is ready to be shutdown, so we're good to use `force` here.
...
begin
  Result := True;
    ...
    if not WizardSilent() and IsAdmin() then begin
      if MsgBox('This User Installer is not meant to be run as an Administrator. If you would like to install Oasis for all users in this system, download the System Installer instead from https://github.com/oasis-editor/oasis. Are you sure you want to continue?', mbError, MB_OKCANCEL) = IDCANCEL then begin
        Result := False;
      end;
    end;
  #endif
...
// Oasis will create a flag file before the update starts (/update=C:\foo\bar)
...
// Check if Oasis created a session-end flag file to indicate OS is shutting down
...
// Check if Oasis created a cancel file to signal that the update should be aborted
...
begin
  ...
  // During background updates, also create a -updating mutex that Oasis checks
...
begin
  ...
  if QualityIsInsiders() and not SessionEndFileExists() and AppxPackageInstalled('Oasis.Editor.Insiders', RemoveAppxPackageResultCode) then begin
    ...
    DeleteFile(ExpandConstant('{app}\appx\oasis_insiders_explorer_{#Arch}.appx'));
    DeleteFile(ExpandConstant('{app}\appx\oasis_insiders_explorer_command.dll'));
  end;
